...
function validatePeople(req, res, next) {
  const { data } = req.body;
  return typeof data.people !== "number"
...
function validStatusChange(req, res, next) {
  ...
  return statuses.includes(status) && currentStatus !== "finished"
    ? next()
    : next({
        status: 400,
        message: `Current status must not be finished, cancelled, nor unknown. Only reservations with current status of 'booked' may be updated. New status must be one of the following valid statuses: ${statuses.join(
          ", "
        )}`,
      });
}
...
function validInitialStatus(req, res, next) {
  const { data } = req.body;
  return data.status && data.status !== "booked"
    ? next({
        status: 400,
        message: `Reservation status may not be 'finished', 'seated', nor 'cancelled.`,
      })
    : next();
}
...
function validUpdateStatus(req, res, next) {
  ...
  return status !== "booked"
    ? next({
        status: 400,
        message:
          "Only reservations with current status of 'booked' may be updated.",
      })
    : next();
}
...
function validateTime(req, res, next) {
  ...
  if (minutes < startingMinutes || minutes > endingMinutes) {
    return next({
      status: 400,
      message: `Please select a time between 10:30 AM and 9:30 PM`,
    });
  }
  next();
}
